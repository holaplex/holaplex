...
const NoFeed: FC = () => {
  ...
  const followMultiple = async() => {
    ...
    const followTx = async(targetPubKey: string) => {
      await graphProgram.methods.makeConnection(new anchor.web3.PublicKey(targetPubKey))
    ...
    const actions: Action[] = [
      {
        name: `Following ${topProfilesToFollow[0].profile?.handle || shortenAddress(topProfilesToFollow[0].address)}`,
        id: `follow${topProfilesToFollow[0].address}}`,
        action: followTx,
        param: topProfilesToFollow[0].address
      },
      {
        name: `Following ${topProfilesToFollow[1].profile?.handle || shortenAddress(topProfilesToFollow[1].address)}`,
        id: `follow${topProfilesToFollow[1].address}}`,
        action: followTx,
        param: topProfilesToFollow[1].address
      },
      {
        name: `Following ${topProfilesToFollow[2].profile?.handle || shortenAddress(topProfilesToFollow[2].address)}`,
        id: `follow${topProfilesToFollow[2].address}}`,
        action: followTx,
        param: topProfilesToFollow[2].address
      },
      {
        name: `Following ${topProfilesToFollow[3].profile?.handle || shortenAddress(topProfilesToFollow[3].address)}`,
        id: `follow${topProfilesToFollow[3].address}}`,
        action: followTx,
        param: topProfilesToFollow[3].address
      },
      {
        name: `Following ${topProfilesToFollow[4].profile?.handle || shortenAddress(topProfilesToFollow[4].address)}`,
  ...
  return (
    ...
    >
      ...
      <Button loading={hasActionPending} onClick={followMultiple}>Follow top 5 profiles</Button>
    </div>
  );
};
